...
const BarajearOpciones = (opciones) => {
  return opciones
...
const BarajearOpcionesExamen = (examen) => {
  examen.preguntas.forEach((p) => {
    p.opciones = BarajearOpciones(p.opciones);
  });
...
export default function QuizViewer() {
  ...
  useEffect(() => {
    fetch("/examen_completo.json")
      .then((res) => res.json())
      .then((data) => {
        // Barajear opciones del examen
        data.examen_1 = BarajearOpcionesExamen(data.examen_1);
        data.examen_2 = BarajearOpcionesExamen(data.examen_2);
        data.examen_3 = BarajearOpcionesExamen(data.examen_3);
        data.examen_4 = BarajearOpcionesExamen(data.examen_4);
        data.examen_5 = BarajearOpcionesExamen(data.examen_5);
        data.examen_6 = BarajearOpcionesExamen(data.examen_6);
  ...
  if (!quizData || !attemptData) {
    return <div className={styles.loading}>Cargando entorno de prueba...</div>;
  }
  ...
  if (!question) {
    return <div className={styles.error}>Pregunta no encontrada.</div>;
  }
  ...
  const hasResult =
    attemptData.results && attemptData.results[questionIndex] !== undefined;
  ...
  const handleFinishExam = () => {
    ...
    exam.preguntas.forEach((q, idx) => {
      ...
      const correctIndices = q.opciones
        .map((o, i) => (o.es_correcta ? i : -1))
        .filter((i) => i !== -1);
      const isOk =
  ...
  return (
    <div className={styles.splitLayout}>
      ...
      <div
        className={`${styles.mainColumn} ${shouldShowExplanation ? styles.panelOpen : ""}`}
      >
        {/* Top Action Bar */}
        <div className={styles.topBar}>
          <div className={styles.examModeInfo}>
            ...
            {isExamMode ? (
              <span className={styles.badgeExam}>MODO EXAMEN</span>
            ) : (
              <span className={styles.badgePractice}>MODO PRÁCTICA</span>
            )}
            {isExamMode && (
        ...
        <div className={styles.card}>
          <div className={styles.header}>
            ...
            <span className={styles.counter}>
              PREGUNTA {questionIndex + 1} DE {exam.preguntas.length}
            </span>
          </div>
          ...
          <h2 className={styles.prompt}>
            ...
            {question.opciones.filter((o) => o.es_correcta).length > 1 && (
              ...
              >
                (Elige {question.opciones.filter((o) => o.es_correcta).length}{" "}
                opciones)
              </span>
            )}
          </h2>

          {question.imagenes_pregunta &&
            question.imagenes_pregunta.length > 0 && (
          ...
          <div className={styles.actions}>
            {!shouldShowExplanation || isExamMode ? (
              ...
              <button className={styles.primaryBtn} onClick={handleNext}>
                {questionIndex < exam.preguntas.length - 1
                  ? "Siguiente Pregunta"
                  : "Finalizar Práctica"}
              </button>
            )}
          ...
          <div className={styles.navigationRow}>
            <button
              ...
              onClick={() =>
                navigate(
                  `/exam/${examId}/attempt/${attemptId}/q/${questionIndex - 1}`,
                )
              }
      ...
      {shouldShowExplanation && !isExamMode && (
        <aside className={styles.sidePanel}>
          <div
            className={`${styles.explanationPane} ${isFullyCorrect() ? styles.paneSuccess : styles.paneError}`}
          >
            <div className={styles.explanationHeader}>
              <h3>
                {isFullyCorrect()
                  ? "Análisis de Solución: Correcta"
                  : "Análisis de Solución: Incorrecta"}
              </h3>
            </div>

            <div className={styles.explanationBody}>
              <p className={styles.mainExplanationText}>
              ...
              <div className={styles.interleavedConcepts}>
                {question.opciones
                  .filter(
                    (o) =>
                      o.es_correcta || (o.imagenes && o.imagenes.length > 0),
                  )
                  .map((opt, idx) => (
                ...
                {question.imagenes_explicacion &&
                  question.imagenes_explicacion.length > 0 && (
